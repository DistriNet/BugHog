...
class TestBiggestGapBisectionSearch:
    ...
    def test_sbg_search_even_available_search(self):
        state_factory = helper.create_state_factory(
            helper.only_has_binaries_for_even, outcome_func=lambda x: True if x < 35 else False
        )
        sequence = BiggestGapBisectionSearch(state_factory)
        ...
        assert [state.index for state in sequence._considered_states] == [0, 24, 30, 32, 34, 36, 48, 98]
    ...
    def test_sbg_search_few_available_search(self):
        state_factory = helper.create_state_factory(
            helper.has_very_few_binaries, outcome_func=lambda x: True if x < 35 else False
        )
        sequence = BiggestGapBisectionSearch(state_factory)
