...
class StateResultFactory:
    ...
    def get_result(self, state_result_data: dict) -> StateResult:
        ...
        is_dirty = (
            not reproduced and not (
                self.__sanity_check_was_successful(state_result_data) or
                self.__sanity_check_was_successful_deprecated(state_result_data)
            )
        )
    ...
    def __is_reproduced_deprecated(self, requests: dict) -> bool:
        ...
        if self.__experiment is None:
            return False
        regex = rf'^https?:\/\/[a-zA-Z0-9-]+\.[a-zA-Z]+\/report\/(\?|\?.+&)leak={self.__experiment}($|&.+$)'
        requests_to_report_endpoint = [request for request in requests if re.match(regex, request['url'])]
        return len(requests_to_report_endpoint) > 0
