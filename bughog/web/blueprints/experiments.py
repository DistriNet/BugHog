...
def before_request():
    ...
    if host not in SUPPORTED_DOMAINS:
        logger.error(f"Host '{host}' is not supported by this framework. Supported hosts are {SUPPORTED_DOMAINS}")
...
def __report(request: Request) -> None:
    ...
    def send_report_to_collector():
        http.post_request(f'http://{remote_ip}:5001/report/', response_data)

    threading.Thread(target=send_report_to_collector).start()


def __get_all_GET_parameters(request) -> dict[str, Any]:
...
def report_endpoint():
    ...
    resp = make_response(render_template('cookies.html', title='Report', get_params=get_params))
...
@exp.route(
    '/<string:project>/<string:experiment>/<string:file_name>.py',
