...
class Interaction:
    ...
    def do_experiment(self, simulation: Simulation) -> None:
        if self._interpret(simulation):
            simulation.sleep(str(simulation.executable.post_experiment_sleep_duration))
        ...
        try:
            for statement in self.script:
                ...
                if method_name not in simulation.supported_commands:
                    raise Exception(f'Invalid command `{cmd}`. Expected one of {", ".join(map(lambda m: m.upper(), simulation.supported_commands))}.')
                ...
                if len(method_params) != len(args) and (len(method_params) < 1 or str(method_params[0])[0] != '*'):
                    raise Exception(f'Invalid number of arguments for command `{cmd}`. Expected {len(method_params)}, got {len(args)}.')
