...
class PublicBrowserStateCache:
    ...
    def update() -> None:
        def safe_request_json_and_update(collection_name: str, transform=lambda x: x):
            ...
            try:
                result = http.fetch_dict(url).get('data')
        ...
        executor.submit(
            safe_request_json_and_update, 'firefox_binary_availability', transform=lambda x: list(x.values())
        )
    ...
    def firefox_get_previous_and_next_commit_nb_with_executable(commit_nb: int) -> tuple[Optional[int], Optional[int]]:
        ...
        previous_commit_nbs = collection.find({'revision_number': {'$lt': commit_nb}}).sort(
