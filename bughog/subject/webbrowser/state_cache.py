...
class PublicBrowserStateCache:
    ...
    def update() -> None:
        ...
        executor.submit(
            safe_request_json_and_update, 'firefox_executable_availability', transform=lambda x: list(x.values())
        )
    ...
    @staticmethod
    def firefox_get_commit_number(commit_id: str) -> int:
        collection = MongoDB().get_collection('firefox_executable_availability')
        result = collection.find_one({'commit_id': commit_id}, {'commit_number': 1})
        if result is None or 'commit_number' not in result:
            raise AttributeError(f"Could not find 'commit_number' in {result}")
        return result['commit_number']

    @staticmethod
    def firefox_has_executable_for(commit_nb: Optional[int]=None, commit_id: Optional[str]=None) -> bool:
        collection = MongoDB().get_collection('firefox_executable_availability')
        if commit_nb:
            result = collection.find_one({'commit_number': commit_nb})
        elif commit_id:
            result = collection.find_one({'commit_number': commit_nb})
        else:
    ...
    def firefox_get_previous_and_next_commit_nb_with_executable(commit_nb: int) -> tuple[Optional[int], Optional[int]]:
        ...
        previous_commit_nbs = collection.find({'commit_number': {'$lt': commit_nb}}).sort({'commit_number': DESCENDING})
        previous_document = next(previous_commit_nbs, None)

        next_commit_nbs = collection.find({'commit_number': {'$gt': commit_nb}}).sort({'commit_number': ASCENDING})
        next_document = next(next_commit_nbs, None)

        return (
            previous_document['commit_number'] if previous_document else None,
            next_document['commit_number'] if next_document else None,
        )
    ...
    @staticmethod
    def get_release_commit_number(browser: str, major_release_version: int) -> int:
        collection = MongoDB().get_collection(PublicBrowserStateCache.__get_release_base_rev_collection(browser))
        if doc := collection.find_one({'major_version': major_release_version}):
            return doc['commit_number']
        raise AttributeError(f"Could not find major release version '{major_release_version}'")

    @staticmethod
    def get_release_commit_id(browser: str, major_release_version: int) -> int:
        collection = MongoDB().get_collection(PublicBrowserStateCache.__get_release_base_rev_collection(browser))
        if doc := collection.find_one({'major_version': major_release_version}):
            return doc['commit_id']
        raise AttributeError(f"Could not find major release version '{major_release_version}'")
