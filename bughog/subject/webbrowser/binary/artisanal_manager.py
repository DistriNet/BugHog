...
class ArtisanalBuildManager:
    ...
    def has_artisanal_binary_for(self, state: State) -> bool:
        return len(list(filter(lambda x: x['id'] == state.commit_nb, self.meta_info))) > 0
    ...
    def add_new_subfolders(self, subfolders):
        ...
        new_subfolders = [
            ...
            if subfolder not in [entry['folder'] for entry in self.meta_info if 'folder' in entry]
        ]
    ...
    def remove_deleted_subfolders(self, subfolders):
        ...
        deleted_subfolders = [
            subfolder
            for subfolder in [entry['folder'] for entry in self.meta_info if 'folder' in entry]
    ...
    def _get_subfolders(self) -> list:
        return [
            subfolder
            for subfolder in os.listdir(self.builds_folder_path)
            if os.path.isdir(os.path.join(self.builds_folder_path, subfolder))
        ]
