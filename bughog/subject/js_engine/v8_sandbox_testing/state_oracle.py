...
class V8SandboxTestingStateOracle(StateOracle):
    """
    State oracle for V8.
    ...
    @Cache.cache_in_db('js_engine', 'v8_sandbox_testing')
    ...
    def get_nearest_state_with_executable(
        self, target_commit_nb: int, lower_bound: int, upper_bound: int,  state_type: Literal['release', 'commit']
    ) -> int | None:
        if target_commit_nb < 92457:
            return 92457  # First commit with available executable
        else:
            # Currently no further lookup mechanism implemented
    ...
    def get_commit_executable_urls(self, commit_nb: int) -> list[str]:
        # Debug:
        return [
            f'https://storage.googleapis.com/v8-asan/linux-release/d8-sandbox-testing-linux-release-v8-component-{commit_nb}.zip?alt=media',
        ]
